...
use sp_runtime::{
    ModuleId,
    traits::{
        SaturatedConversion, Zero, AccountIdConversion
    },
};
use sp_std::prelude::*;
use sp_core::H160;
...
decl_storage! {
    trait Store for Module<T: Config> as Nft {
        ...
        pub BalanceItem get(fn balance_item_count): double_map hasher(blake2_128_concat) NftGroup, hasher(blake2_128_concat) T::AccountId => u64;

        /// Lock amount owner per collection map
        pub LockedItem get(fn locked_count): double_map hasher(blake2_128_concat) NftGroup, hasher(blake2_128_concat) T::AccountId => u64;
...
decl_module! {
    pub struct Module<T: Config> for enum Call where origin: T::Origin {
        ...
        pub fn accept_sale_order(origin, collection_id: u64, item_id: u64) -> DispatchResult {
            ...
            <T as Config>::MultiCurrency::transfer(currency_id, &sender, &nft_owner, price.saturated_into())?;
        ...
        pub fn accept_separable_sale_order(origin, order_id: u64, value: u64) -> DispatchResult {
            ...
            <T as Config>::MultiCurrency::transfer(currency_id, &sender, &nft_owner, checked_value.into())?;
        ...
        pub fn bid(origin, collection_id: u64, item_id: u64) -> DispatchResult {
            ...
            let free_balance = <T as Config>::MultiCurrency::free_balance(currency_id, &sender);
            ensure!(free_balance > price.into(), "Insufficient balance");

            let lock_id = Self::auction_lock_id(auction.id);
            <T as Config>::MultiCurrency::extend_lock(lock_id, currency_id, &sender, price.into())?;
        ...
        pub fn finish_auction(origin, collection_id: u64, item_id: u64) -> DispatchResult {
            ...
            if let Some(winner) =  histories.last() {
                ...
                let lock_id = Self::auction_lock_id(auction.id);
                <T as Config>::MultiCurrency::remove_lock(lock_id, currency_id, &winner.bidder)?;
                <T as Config>::MultiCurrency::transfer(currency_id, &winner.bidder, &auction.owner, winner.bid_price.into())?;

                for i in 0..(histories.len() - 1) {
                    let h = &histories[i];
                    <T as Config>::MultiCurrency::remove_lock(lock_id, currency_id, &h.bidder)?;
                }
...
impl<T: Config> Module<T> {
    ...
    fn add_fungible_item(item: FungibleItemType<T::AccountId>) -> DispatchResult {
        ...
        let nft_group = NftGroup {
			collection: item.collection,
			item: current_index
		};
		// Update balance item
		<BalanceItem<T>>::insert(nft_group, owner.clone(), value);
    ...
    fn add_nft_item(item: NftItemType<T::AccountId>) -> DispatchResult {
		...
		// Update balance item
		<BalanceItem<T>>::insert(nft_group, item_owner.clone(), 1);
...
impl<T: Config> NftManager<T::AccountId, T::BlockNumber> for Module<T> {
	...
	fn unlock_fungible(
		...
		let locked_owner = <Balance<T>>::get(collection_id, owner.clone())
			.checked_sub(lock_value)
			.unwrap();
		<Locked<T>>::insert(collection_id, owner.clone(), locked_owner);

		let balance_owner = <Locked<T>>::get(collection_id, owner.clone())
			.checked_add(lock_value)
			.unwrap();
    ...
    fn charge_royalty(buyer: T::AccountId, collection_id: u64, item_id: u64, currency_id: CurrencyId, order_price: u64, now: T::BlockNumber) -> DispatchResult {
        ...
        if royalty.expired_at >= now && royalty.rate >= Zero::zero() {
            ...
            <T as Config>::MultiCurrency::transfer(currency_id, &buyer, &royalty.owner, royalty_fee.into())?;
        }
        Ok(())
    }
}
